...
- **运行中徽章**：内核执行任务时，输出区右上角悬浮显示红色脉冲圆点 +「运行中 mm:ss」实时耗时；任务结束自动消失。长任务/大计算即使暂无输出也能一眼看出程序仍在运行，避免误以为已跑完而重复执行。

## [1.4.0] - 2026-08-27

### 新增

- **输出配色自适应当前主题**：`output*` 颜色未显式设置时，按当前主题编辑器背景自动派生——背景分级往白偏移（暗底微提亮 5% 成暗灰、中间浅色明显变淡 15%、近白大幅白化以拉开与编辑器底色的区分度，方向只有往白、绝不往深走）；前景/代码/print 输出按背景明暗对撞选色（暗底亮字、亮底深字，含对比度兜底保证可读）；ANSI 16 色与 In/Out 提示符按浅色/深色各一套适配。用户显式设置的任一 `output*` 颜色始终优先；高对比度主题不偏移；切换主题实时跟随。
- **README 新增常用配色方案**：白色（qtconsole 经典）/ Dracula / One Dark / Solarized 深、浅共 5 套完整 `output*` 配色，可直接整块复制进 `settings.json`。
